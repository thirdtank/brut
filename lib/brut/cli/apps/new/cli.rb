...
module Brut::CLI::Apps::New
  class CLI
    ...
    def run
      ...
      if command == :new_app
        app_options = parse_options(@args, Brut::CLI::Apps::New::Versions.new)
        new_app = Brut::CLI::Apps::New::App.new(
          current_dir: Pathname.pwd.expand_path,
          app_options:,
          out: PrefixedIO.new(@out, "brut new"),
          err: @err
        )
        new_app.create!
      elsif command == :add_segment
        add_segment_options = parse_add_segment_options(@args, Brut::CLI::Apps::New::Versions.new)
        add_segment = Brut::CLI::Apps::New::AddSegment.new(
  ...
  private
    ...
    def parse_options(args, versions)
      ...
      @option_parser = OptionParser.new do |opts|
        opts.accept(Brut::CLI::Apps::New::Prefix) do |prefix|
          Brut::CLI::Apps::New::Prefix.new(prefix)
        end
        opts.accept(Brut::CLI::Apps::New::AppId) do |prefix|
          Brut::CLI::Apps::New::AppId.new(prefix)
        end
        opts.accept(Brut::CLI::Apps::New::Organization) do |prefix|
          Brut::CLI::Apps::New::Organization.new(prefix)
        end
        opts.banner = [
          "Usage: brut new [options] app-name",
          "",
          "    Creates a new Brut-powered app",
          "",
          "VERSION",
          "",
          "    #{Brut::CLI::Apps::New::VERSION}",
          "",
          "OPTIONS",
          "",
        ].join("\n")

        opts.on("-a", "--app-id=ID", Brut::CLI::Apps::New::AppId,
        ...
        opts.on("-o", "--organization=ORG",Brut::CLI::Apps::New::Organization,
                "Organization name, e.g. what you'd use for GitHub. Defaults to the app-id value")

        opts.on("-e", "--prefix=PREFIX", Brut::CLI::Apps::New::Prefix,
      ...
      options[:app_name] = Brut::CLI::Apps::New::AppName.new(args.first)
      options[:app_id] = options[:'app-id']
      options[:dry_run] = !!options[:'dry-run']
      Brut::CLI::Apps::New::AppOptions.new(**options, versions:)
    end
    ...
    def parse_add_segment_options(args, versions)
      ...
      @option_parser = OptionParser.new do |opts|
        opts.banner = [
          "Usage: brut new add-segment [options] segment-name",
          "",
      ...
      Brut::CLI::Apps::New::AddSegmentOptions.new(segment_name: @args[0], **options, versions:)
    end
  end
end 
