...
middleware. Its configuration requires that *all* HTTP POST requests include a valid CSRF token, with two exceptions: Brut-owned paths and webhooks.  See the [Technical Notes](#technical-notes) section below.
...
CSRF protection is also skipped for any handler defined via `webhook`:
...
class App < Brut::Framework::App
  ...
  webhook "sendgrid" # => /webhooks/sendgrid doesn't require a CSRF token
end
```
